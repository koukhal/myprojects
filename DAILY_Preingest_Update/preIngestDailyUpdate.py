...
Created on 24 oct. 2016
 ...
 Modif date     : 03/11/2016 : Ajout d'un repertoire pour les paquets d'update
                ...
                : 28/09/2017 : Generate NACK (METS XML not valid/ METS not found)
...
def sendEMail(message, sip):
    try:
        ...
        # Prepare the message
        Subject = "Pre-Ingest Module - UPDATE DAILY : %s (%s)" % (sip,sipsite)
...
def sendEMailold(message,sip):
    try :
        ...
        # Prepare the message
        Subject = "Pre-Ingest Module TEST - DAILY UPDATE(%s): %s" %(sipsite,sip)
        message = textwrap.dedent("""\
        From: %s
        To: %s
        Subject: %s
...
def readMETSXML(xmlFile,sip):
    ...
    try :
        ...
        elif 'catl' in dicoids:                           
       
                cellarId = dicoids.get('cellar')
                collection=dicoids.get('catl')
                collectionTodir = collection
        ...
        try :
            ...
            ts = time.time()
            fdate = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
            print fdate + ' : %s - %s' %(e.diag.message_primary,e.diag.context)
            logger.error('%s' % e)
            sendEMail("Error during to insert of METS INFO  %s -%s " %(e.diag.message_primary,e.diag.context),sip) 
            return False
    ...
    try :
        ...
        for item in (listExpression):
            ...
            if 'oj' in dicoExpression :
                ...
                collection = dicoExpression.get('immc')
                # publang = collection.split('.')[1]
                lglist = []
                for line in collection.split('.'):
                    lglist.append(line)
                ...
                if 'celex' in dicoExpression:
                    ...
                    if premkey == 'cellar':
                        premval = list(dicoExpression.values())[1]
                        premkey = list(dicoExpression.keys())[1]
                    
                    #print "premkey %s - premval %s"%(premkey,premval)
                    
                    collection = dicoExpression.get(premkey)
                    #print "COLECTION : %s"%collection
...
def storeManifestation(listManifestation):
    ...
    try :
        ...
        for item in (listManifestation):
            ...
            if 'oj' in dicoManifestation :
            ...
            elif 'immc'in dicoManifestation:
                ...
                collection = dicoManifestation.get('immc')
                ...
                if celex != None:
                    ...
                    #print "Collection %s premkey %s premval %s"%(collection,premkey,premval)
            ...
            if count == 0:
                ...
                updateQuery = "UPDATE manifestation SET lastupdate = current_timestamp , collection ='%s'"%collection
                #updateQuery += " where collection ='%s' and fileid = '%s' and filename ='%s' " % (collection,fileid,filename)
                updateQuery += " where fileid = '%s' and filename ='%s' " % (fileid,filename)
                logger.info('Execute Query : %s' % updateQuery)
                #print "Update ", updateQuery
                cur.execute(updateQuery)
                con.commit()
...
def controlSIP(sip_path):
    ...
    try:
        ...
        Query += "VALUES ('%s','%s','INPROGRESS','%s','%s','DAILY','%d','%d','%s','%s')" % (sip,receiptdate,sipsite,sipserver,sizeinbyte,totalFiles,creationDate,lastModification)
...
print "    pre-Ingest Daily - Update instance"
print "----------------------------------------------"
        ...
        try:
            ...
            if retcontrol == False :
                ...
                if (packagetype == 'create') :
                    #endout = serieTodest+"//"+timestamp+"_"+collectionTodir             
                    ...
                    #endout = serieTodest+"//"+timestamp+"_"+collectionTodir             
                    endout = timestamp+"_"+collectionTodir 
                    outdir =os.path.join(waitdir,endout)
                    print fdate +" : Move %s to %s" %(sip,outdir)
                    moveSIP(workdir,outdir,sip)
                    ready=os.path.join(outdir,".ready")               
