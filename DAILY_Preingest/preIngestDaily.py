...
Created on 24 oct. 2016
 ...
 Name          : preIngestDaily.py (create)
 Version       : 3.0
 Modif date    : 03/11/2016 : Ajout d'un repertoire pour les paquets d'update
               : 17/11/2016 : Isoler les update vers une autre instance
               : 13/12/2016 : Modifier les insertions/update dans la base de donnees dans le cas des rejets (METS non present ...)
               : 05/01/2017 : Ne pas traiter les paquets genpub et les deplacer vers un repertoire temporaire pour supression
               : 14/02/2017 : Faire un insert au lieu d'un update lors d'une erreur avec le nombre de clefs dans le contentids
               : 21/02/2017 : Retraitrer les GENPUB
               : 09/08/2017 : Correction multiple keys
               : 19/09/2017 : Generate NACK (METS XML not valid/ METS not found)
               : 19/10/2017 : Correction of pi_com package
               : 15/12/2017 : Check if idcellar exists
               : 22/12/2017 : Add nack for all controls about the package (opening, METS Validation, total files in sip ...)
               : 18/01/2018 : Ajout de la collection CONSIL dans le traitement
               : 24/01/2018 : Traitement des Expressions dans le cas ou le type de collection n'est pas reconnue
               : 03/04/2018 : Ajout des collections legissum
...
def sendEMail(message, sip):
    try:
        ...
        # Prepare the message
        Subject = "Pre-Ingest Module - DAILY (%s): %s" % (sipsite,sip)
        ...
        The module preIngest on %s (%s) has encountered the following issue :
        ...
        EUDOR Operators,
        admEudor@intrasoft-intl.com.

        """ % (smtp_login, destinataire, Subject, 2, sipsite,sipserver,message))
...
def parseConfig(configFile):
    try:
        ...
        updrep = doc.getElementsByTagName("updatedir")[0]
        global updir
        updir = getNodeText(updrep)
        
        pubdelrep = doc.getElementsByTagName("pubdeldir")[0]
        global pubdeldir
        pubdeldir = getNodeText(pubdelrep)
        ...
        nackrep = doc.getElementsByTagName("nackdir")[0]
        global nackdir
        nackdir = getNodeText(nackrep)
...
def checkWorkexists(idcellar, collectiontocheck):
    ...
    # except psycopg2.DatabaseError, e:
        ...
        ts = time.time()
        fdate = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
        print fdate + " : Error %s - %s" %(e.diag.message_primary,e.diag.context)
        logger.error('Error during a database transaction')
        logger.error('%s ' % e)
        sendEMail("Error during a database transaction %s - %s : " %(e.diag.message_primary,e.diag.context), sip)
...
def compareTotalFiles(sip):
    ...
    if cmp != 0:
        ts = time.time()
        fdate = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
        print fdate + " : Difference between files referenced in METS and presents in ZIP %d/%d " % (
        totalFilesinMETS, (totalinzip - 1))
        logger.info('Difference between files referenced in METS and presents in ZIP (%d)' % cmp)
...
def storeSIPtoDB(sip, Query):
    ...
    try:
        ...
        ts = time.time()
        fdate = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
        print fdate + " : Store package %s in Database" % sip
        ...
        if count == 0:
            ...
            nack("none",sip,"SIP package was already processed.","EUDOR-ERROR-606")
            return False
...
def openingZip(zfile):
    ...
    entryZIP= ZipFile(zfile)

    try:
        # Check ZIP file
        retval=entryZIP.testzip()
        ...
        if retval is not None:
            ...
            # update database
            Query = "INSERT INTO infopackage (packagename, receipt_date,process_date,status,description,site,server,service) "
            Query += "VALUES ('%s','%s','%s','ERROR','Error during the opening of SIP.','%s','%s','DAILY')" % (
                sip, dateOfReception, fdate, sipsite, sipserver)

            retins = insertQuery(Query)
            if retins == False:
                ts = time.time()
                fdate = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
                print fdate + " : 'Error during to the insert of the DB  - %s" % sip
...
def dezipMETS(metsFile, outDir, sip):
    try:
        ...
        if os.path.isfile(metsXMLPath) == False:
            ...
            logger.error('The METS file %s doesnt exist in SIP .' % metsXML)
            sendEMail("The METS file %s doesnt exist in SIP ." %metsXML, sip)
            
            Query = "INSERT INTO infopackage (packagename, receipt_date,process_date,status,description,site,server,service) "
            Query += "VALUES ('%s','%s','%s','ERROR','The METS file doesnt exist in SIP.','%s','%s','DAILY')" % (sip, dateOfReception, fdate, sipsite, sipserver)
        ...
        # update database
        Query = "INSERT INTO infopackage (packagename, receipt_date,process_date,status,description,site,server,service) "
        Query += "VALUES ('%s','%s','%s',ERROR','Error to uncompress the package.','%s','%s','DAILY')" % (
            sip, dateOfReception, fdate, sipsite, sipserver)
...
def readMETSXML(xmlFile, sip):
    ...
    try:
        ...
        # Get Cellar ID and Collection from METS info
        contentids_wk = tupMetsInfo[4]
        dicochek = {}

        for item in (contentids_wk).split(' '):
        ...
        if 'oj' in dicochek:
            cellarIdTochek = dicochek.get('cellar')
            collectionTodir = dicochek.get('oj')

        elif 'com' in dicochek:
            cellarIdTochek = dicochek.get('cellar')
            print fdate + ' : %s' % cellarIdTochek
            collectionTodir = dicochek.get('com')
            
        elif 'genpub' in dicochek:
            
            cellarIdTochek = dicochek.get('cellar')
            collectionTodir = dicochek.get('genpub')
            
            #return None

        else:
            cellarIdTochek = dicochek.get('cellar')
            celex = dicochek.get('celex')
            comnat = dicochek.get('comnat')
            uriserv = dicochek.get('uriserv')
            ...
            elif cellarIdTochek is not None:
                collectionTodir = cellarIdTochek
            else:
                collectionTodir = "notdefined"

        if cellarIdTochek is None :
            #print "************ %s" %cellarIdTochek
            return "NoneID"
...
# Store METS information
def StoreMETS(xmlFile, sip):
    serie = ""
    idnum = ""
    idyear = ""
    idext = ""
    ...
    try:
        ...
        metsdocid, metsprofile, serie, idyear, idnum, idext, pubvol, publang, metstype, metscreatedate)
...
def storeToDB(dicToStore, tableToStore, metsdocid):
    ...
    try:
            ...
            if len(dicToStore.keys()) != 0:
                while j < len(v):
                    ...
                    for i in range(len(dicToStore.keys())):
                        ...
                        values_tostore = "('" + "','".join(map(str,
                                                               listvaleur)) + "', (select metsdocid from metsinfo where metsdocid ='%s') )" % metsdocid
        ...
        logger.error('%s' % e)
        sendEMail("Error during the insertion in the DB of the data from METS XML: %s -%s " %(e.diag.message_primary,e.diag.context), metsdocid)
        con.rollback()
        con.close()
        return False
...
def storeWorkPub(listWork):
    ...
    try:
        ...
        if 'oj' in dicoWork:
        ...
        elif 'jure' in dicoWork:

            cellarId = dicoWork.get('cellar')
            collection = dicoWork.get('jure')
            typeOfcollection = "jure"
            serie = "jure"
            idyear = ""
            idnum = ""
            ...
            cellarId = dicoWork.get('cellar')
            collection = dicoWork.get('catl')
            typeOfcollection = "catl"
            splitpub = collection.split('_')
            serie = splitpub[0]
            idyear = splitpub[1]
            idnum = splitpub[2]
        ...
        ts = time.time()
        fdate = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
        print fdate +" : ContentIDS : %s- CellarID : %s" %(contentids,cellarId)
        ...
        if count == 0:
            ...
            insertQuery += "VALUES ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (
            dmdidWork, contentids, cellarId, celex, collection, serie, idyear, idnum, idext, typeOfcollection, comnat,
            uriserv)
            logger.info('Execute Query : %s' % insertQuery)
            # print insertQuery
            cur.execute(insertQuery)
        ...
        logger.error('%s' % e)
        sendEMail("WORK insertion failed: %s -%s" %(e.diag.message_primary,e.diag.context), sip)
        con.rollback()
        con.close()
        return False
    ...
    try:
        ...
        for item in (listExpression):
            ...
            if 'oj' in dicoExpression:
                ...
                collection = dicoExpression.get('jure')
                # publang = collection.split('.')[1]
                lglist = []
                for line in collection.split('.'):
                    lglist.append(line)
                ...
                collection = dicoExpression.get('catl')
                # publang = collection.split('.')[1]
                lglist = []
                for line in collection.split('.'):
                    lglist.append(line)
            ...
            if count == 0:
                ...
                insertQuery += "VALUES ('%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (
                dmdidExpression, contentids, cellarId, idext, collection, publang, comnat, celex, uriserv)
                logger.info('Execute Query : %s' % insertQuery)
                # print insertQuery
                cur.execute(insertQuery)
                con.commit()
        ...
        logger.error('%s' % e)
        sendEMail("EXPRESSSION insertion failed: %s - %s" %(e.diag.message_primary,e.diag.context), sip)
        con.rollback()
        con.close()
        return False
...
def storeManifestation(listManifestation):
    ...
    try:
        ...
        for item in (listManifestation):
            ...
            if 'oj' in dicoManifestation:
                ...
                if dicoManifestation.has_key('fileid') is True:
                    ...
                    # print "collection %s - cellarid % ext %s - %s - %s - %s" % (collection,cellarId,idext,fileid,doctype,filename)
            ...
            elif 'jure' in dicoManifestation:
                if dicoManifestation.has_key('cellar') is True:
                    cellartmp = dicoManifestation.get('cellar')
                    splitcellar = cellartmp.split('.')
                    cellarId = splitcellar[0]
                ...
                collection = dicoManifestation.get('jure')
            ...
            elif 'catl' in dicoManifestation:
                if dicoManifestation.has_key('cellar') is True:
                    cellartmp = dicoManifestation.get('cellar')
                    splitcellar = cellartmp.split('.')
                    cellarId = splitcellar[0]
                ...
                collection = dicoManifestation.get('catl')
                ...
                if celex != None:
                    ...
                    #print" Key %s VAL %s" %(premkey,premval)
                    
                    if premkey == 'cellar':
                        premval = list(dicoManifestation.values())[1]
                        premkey = list(dicoManifestation.keys())[1]
                    
                    #print "Prem KEY %s PREM VAL %s"%(premkey,premval)
                    
                    collection = dicoManifestation.get(premkey)
                    print "Collection %s"%collection
                    
                    checkpublang = collection.split('.')[1]
                    print "LANG %s"%checkpublang
                    if len(checkpublang) == 3:
                        publang = checkpublang
                        doctype = collection.split('.')[2]
                    elif len(checkpublang) > 3:
            ...
            if count == 0:
                ...
                insertQuery += "VALUES ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (
                contentids, cellarId, idext, collection, publang, doctype, filename, fileid, comnat, celex, uriserv)
                logger.info('Execute Query : %s' % insertQuery)
                #print insertQuery
                cur.execute(insertQuery)
                con.commit()
                ...
                updateQuery += " where collection ='%s' and fileid = '%s' and filename ='%s' " % (
                collection, fileid, filename)
                logger.info('Execute Query : %s' % updateQuery)
                # print "Update ", updateQuery
                cur.execute(updateQuery)
                con.commit()
        ...
        logger.error('%s' % e)
        sendEMail("MANIFESTATION insertion failed: %s - %s" %(e.diag.message_primary,e.diag.context), sip)
        con.rollback()
        con.close()
        return False
...
def controlSIP(sip_path):
    ...
    try:
        ...
        fdate = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
        receiptdate = fdate
        global dateOfReception
        dateOfReception = receiptdate
        ...
        if retopenzfile == False:
            ...
            nack("none",sip,"Checking of opening SIP failed.","EUDOR-ERROR-601")
            return False
        ...
        if retdezipmets == False:
            ...
            nack("none",sip,"METS XML not found","EUDOR-ERROR-602")
            return False
        ...
        if retvalschema == False:
            ...
            print fdate + " : Workdir Cleaning ."
            removeFiles(workdir, sip)
            nack("none",sip,"Error during the METS Validation","EUDOR-ERROR-603")
            return False
        ...
        if retreadxml == False:
            ...
            Query = "INSERT INTO infopackage (packagename, receipt_date,process_date,status,description,site,server,service,packagesize,nbfiles,last_modification) "
            Query += "VALUES ('%s','%s','%s',ERROR','Error during the METS XML Parsing.','%s','%s','DAILY','%d','%d','%s')" % (sip, dateOfReception, fdate, sipsite, sipserver, sizeinbyte, totalFiles, lastModification)
            ...
            nack("none",sip," Error during the METS XML Parsing","EUDOR-ERROR-604")
            return False
        elif retreadxml == "NoneID" :
                ts = time.time()
                fdate = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
                print fdate + " : Move %s to %s" % (sip, rejdir)
                moveSIP(workdir, rejdir, sip)
                print fdate + " : Workdir Cleaning ."
                removeFiles(workdir, sip)
                nack("none",sip,"ID not found in the work contentids.")
                Query = "INSERT INTO infopackage (packagename, receipt_date,process_date,status,description,site,server,service,packagesize,nbfiles,last_modification) "
                Query += "VALUES ('%s','%s','%s','ERROR','ID not found in the work contentids.','%s','%s','DAILY','%d','%d','%s')" % (sip, dateOfReception, fdate, sipsite, sipserver, sizeinbyte, totalFiles, lastModification)
        ...
        if retreadxml == None:            
            ...
            fdate = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
            processdate = fdate
            print fdate + " : GENPUB detected %s " % collectionTodir
            # move file to rejected
            print fdate + " : Move %s to %s" % (sip, pubdeldir)
            moveSIP(workdir, pubdeldir, sip)
            print fdate + " : Workdir Cleaning ."
            removeFiles(workdir, sip)   
            ignore ="IGNORE"     
            return ignore
        ...
        retcheck = checkWorkexists(cellarIdTochek, collectionTodir)
        if retcheck == True:
            ts = time.time()
            fdate = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
            print fdate + " : WORK %s exists " % collectionTodir
            # move file to rejected
            print fdate + " : %s must be processed as UPDATE " %sip
            print fdate + " : Move %s to %s" % (sip, updir)
            moveSIP(workdir, updir, sip)
            print fdate + " : Workdir Cleaning ."
            removeFiles(workdir, sip)
            ...
            ts = time.time()
            fdate = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
            print fdate + " : WORK %s doesnt exist " % collectionTodir
            ...
            Query += "VALUES ('%s','%s','INPROGRESS','%s','%s','DAILY','%d','%d','%s','%s')" % (
            sip, receiptdate, sipsite, sipserver, sizeinbyte, totalFiles, creationDate, lastModification)
            ...
            # Insert METS Information
            storemets = StoreMETS(metsXML, sip)
            if storemets == False:
                ts = time.time()
                fdate = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
                processdate = fdate
                print fdate + " : Error during the METS XML Insertion in DB %s " % sip
                ...
                # update database
                Query = "UPDATE infopackage SET status ='ERROR', description ='Error during the METS Insertion . "
                Query += " ,process_date ='%s' " % processdate
                Query += " where packagename ='%s' " % sip
                retupd = updateDB(Query)
                if retupd == False:
                ...
                nack("none",sip,"An internal system error has occurred please contact EUDOR operators.","EUDOR-ERROR-610")
                return False
            ...
            if valOfCmp != 0:
                ...
                print fdate + " : Workdir Cleaning ."
                removeFiles(workdir, sip)
                nack("none",sip,"A difference of files have been detected between METS XML and SIP.","EUDOR-ERROR-605")

                # update database
                QueryToExec = "UPDATE infopackage SET status ='ERROR', "
                QueryToExec += " description ='A difference of files have been detected between METS XML and %s (%d)' " % (
                sip, valOfCmp)
                ...
                if len(DMDdic) != 0:
                    ...
                    if retstoredb == False:
                        ...
                        if retupd == False:
                            ts = time.time()
                            fdate = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
                            print fdate + " : 'Error during the update transaction in DB  - %s" % sip
                            logger.error('Error during the update transaction in DB - %s' % sip)
                            sys.exit(1)
                ...
                if retstoreWorkPub == False:
                    ...
                    fdate = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
                    processdate = fdate
                    print fdate + " : Error during the insertion of the WORK %s " % sip
                    # move file to rejected
                    print fdate + " : Move %s to %s" % (sip, rejdir)
                    moveSIP(workdir, rejdir, sip)
                    print fdate + " : Workdir Cleaning ."
                ...
                if retStoreExpression == False:
                    ...
                    fdate = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
                    processdate = fdate
                    print fdate + " : Error during the insertion of the Expression %s " % sip
                    # move file to rejected
                    print fdate + " : Move %s to %s" % (sip, rejdir)
                    moveSIP(workdir, rejdir, sip)
                    print fdate + " : Workdir Cleaning ."
                ...
                if retStoreManifestation == False:
                    ...
                    fdate = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
                    processdate = fdate
                    print fdate + " : Error during the insertion of the MANIFESTATION %s " % sip
                    # move file to rejected
                    print fdate + " : Move %s to %s" % (sip, rejdir)
                    moveSIP(workdir, rejdir, sip)
                    print fdate + " : Workdir Cleaning ."
                ...
                endout = timestamp + "_" + collectionTodir
                outdir = os.path.join(destination, endout)
                print fdate + " : Move %s to %s" % (sip, outdir)
                moveSIP(workdir, outdir, sip)
                ready = os.path.join(outdir, ".ready")
        ...
        # update database
        Query = "INSERT INTO infopackage (packagename, receipt_date,process_date,status,description,site,server,service) "
        Query += "VALUES ('%s','%s','%s','ERROR','%s','%s','%s','DAILY')" % (
                sip, dateOfReception, fdate, e, sipsite, sipserver)
...
# Generate NACK
#
def nack(cellarid,sip,error,code):
    ts = time.time()
    nackdate = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
    ...
    # Write JSON file  
    tmpfile = os.path.splitext(os.path.basename(sip))[0]
    jsonfile=os.path.join(nackdir, tmpfile+".json")
    with io.open(jsonfile, 'w', encoding='utf8') as outfile:
        str_ = json.dumps(data,indent=4)
        outfile.write(to_unicode(str_))
    
    #Create ZIP File
    zipname = os.path.join(nackdir,sip)    
    subprocess.check_output(["zip", "-j" ,zipname,jsonfile])
    
    #Remove file except ZIP
    removeFiles(nackdir, sip)
...
print "    pre-Ingest Daily - CREATE Instance"
print "----------------------------------------------"
...
if sys.argv[1] == "-v":
    print " %s - version %s" %(os.path.basename(str(sys.argv[0])),version)
    print "----------------------------------------------"
    print "----------------------------------------------"
    sys.exit(0)
...
while continuer:
    ...
    if retscandir == False:
        ...
        fdate = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M:%S')
        print "-----------------------------------------------------------"
        print fdate + " End of processing %s ---" % os.path.basename(sip_paquet)
        print "-----------------------------------------------------------"
    elif retscandir == None:
        print ".",
        time.sleep(tempo)
